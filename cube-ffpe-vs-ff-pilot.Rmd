...
```{r}
long.dataset.names <- c('SC2200108_DT-21-4142-BR6526', 'SC2200109_DT-21-4142-BR6526', 'SC2200110_DT-21-4141-BR7212', 'SC2200111_DT-21-4141-BR7212')
...
plot.hne <- function(obj) {
  ...
  g <- g + theme(plot.title = element_text(hjust = 0.5), text = element_text(size = 15), legend.position = "none")
  g
}
```
...
hne.plots <-
  llply(datasets,
        .fun = function(dataset) {
          g <- plot.hne(unfiltered.objs[[dataset]])
          g <- g + ggtitle(dataset)
          g
        })
...
plot.distributions.vs.cell.type <- function(df, response.var = "nCount_Spatial", facet.var = "sample.name", dependent.var = "spot_type") {
  stat.test <- tbl %>% group_by_at(facet.var) %>% wilcox_test(as.formula(paste0(response.var, " ~ ", dependent.var)), p.adjust.method = "none")
  ...
  g <- ggviolin(tbl, dependent.var, response.var, facet.by = facet.var, add = "boxplot")
...
g.cnt <- plot.distributions.vs.cell.type(tbl, response.var = "nCount_Spatial") + ylab("UMI Counts (log2)")
g.feature <- plot.distributions.vs.cell.type(tbl, response.var = "nFeature_Spatial") + ylab("Feature Counts (log2)")
...
g <- ggplot(downsample.feature.df, aes(x = downsample.count, y = num.features, colour = sample.name))
g <- g + geom_point()
...
top.genes.background.plts <-
  llply(datasets,
        .fun = function(data.set) {
          ...
          g <- plot.top.genes(background.sub.matrix, n.top = 20, highlight.genes = common.genes)
          g <- g + ggtitle(data.set)
          g
        })
...
top.genes.tissue.plts <-
  llply(datasets,
        .fun = function(data.set) {
          ...
          g <- plot.top.genes(tissue.sub.matrix, n.top = 20, highlight.genes = common.genes)
          g <- g + ggtitle(data.set)
          g
        })
