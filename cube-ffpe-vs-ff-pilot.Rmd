...
g <- ggplot(data = subset(summary.tbl, metric == "Fraction Reads in Spots Under Tissue"), aes(x = sample, y = value, fill=type)) + geom_col() 
...
g <- ggplot(data = subset(summary.tbl, !(metric %in% c("Fraction Reads in Spots Under Tissue", "Number of Spots Under Tissue"))), aes(x = sample, y = value, fill=type)) + geom_col() + facet_wrap(~ metric, scales="free", labeller = label_wrap_gen())
g <- g + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
g <- g + ylab("")
...
obj.sct <- FindNeighbors(obj.sct, dims = 1:20)
obj.sct <- FindClusters(obj.sct, verbose = FALSE)
obj.sct <- RunUMAP(obj.sct, dims = 1:20)
png(paste0(plots_dir,"clusters-merged-UMAP-sct.png"),width=1400,height =600)
gg <- DimPlot(obj.sct, reduction = "umap", group.by = c("ident", "orig.ident"))
...
png(paste0(plots_dir,"all-cluster-proportions-heatmap-scran.png"))
Heatmap(acast(bar[, c("cluster","sample","frac")], cluster ~ sample),row_title="Cluster",column_title="Sample",heatmap_legend_param=list(title="Proportion"))
d <- dev.off()
...
gb <- getBM(attributes=c("external_gene_name", "gene_biotype"),filters = c("external_gene_name"), values=frozen.only, mart=mouse)
...
obj.scran <- FindNeighbors(obj.scran, dims = 1:20)
obj.scran <- FindClusters(obj.scran, verbose = FALSE)
obj.scran <- RunUMAP(obj.scran, dims = 1:20)
png(paste0(plots_dir,"clusters-merged-UMAP-scran.png"),width=1400,height =600)
gg <- DimPlot(obj.scran, reduction = "umap", group.by = c("ident", "orig.ident"))
gg <- gg + theme(text = element_text(size = 20))
print(gg)
d <- dev.off()
