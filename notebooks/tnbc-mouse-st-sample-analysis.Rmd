...
```{r}
analysis.dir <- "/projects/activities/jaxcc/CATch-project/analysis/tnbc/palucka-catch-subset/"
dir.create(analysis.dir)
plot.dir <- paste0(analysis.dir, "/plots/")
dir.create(plot.dir)
```
...
define.tnbc.metadata <- function() {
  ...
  patient.df$slide <- NA
  flag <- (patient.df$patient %in% c("9990", "7031")) & (patient.df$label == "Batch 1")
  patient.df[flag,"slide"] <- 1
  flag <- (patient.df$patient %in% c("4700", "2171")) & (patient.df$label == "Batch 1")
  patient.df[flag,"slide"] <- 2
  flag <- (patient.df$patient %in% c("9990", "1127", "1330A", "1863B")) & !(patient.df$label == "Batch 1")
  patient.df[flag,"slide"] <- 3
  flag <- (patient.df$patient %in% c("4700", "6110", "6970", "4401B")) & !(patient.df$label == "Batch 1")
  patient.df[flag,"slide"] <- 4
...
# Extract the sample name from the datasets (i.e., foo_<sample>)
ret <- define.tnbc.metadata()
patient.df <- ret$patient.df
...
# Keep the samples we are considering for CATch3
patient.df$brca1.status <- NA
samples <- subset(patient.df, patient %in% c("1127", "1330A", "1863B", "6110", "6970", "4401B"))$sample
patient.df[samples, "brca1.status"] <- "WT"
patient.df[subset(patient.df, patient %in% c("1330A"))$sample, "brca1.status"] <- "MT"
patient.df$name <- paste0(patient.df$patient, " (", patient.df$brca1.status, ")")
# 1863B has two technical replicates -- one was resequenced at a higher depth. 
samples <- samples[!(samples %in% "SC2300231_JP1863B")]
# Only keep that one.
names(samples) <- samples
spaceranger_dirs <- spaceranger_dirs[samples]
datasets <- samples
```
...
scts <- 
  llply(datasets, .parallel = FALSE,
        .fun = function(dataset) {
          rds.file <- paste0(analysis_dir, "/", dataset, "-sct.rds")
          print(rds.file)
          if(!file.exists(rds.file)) {
...
xcells <- 
  llply(datasets, .parallel = FALSE,
        .fun = function(dataset) {
          print(dataset)
          rds.file <- paste0(analysis_dir, "/", dataset, "-xcell.rds")
          print(file.exists(rds.file))
          if(!file.exists(rds.file)) {
            # Note that xCell works on ranks, not values, so
            # normalization doesn't matter
            mat <- GetAssayData(filtered.objs[[dataset]], slot="counts")
            xc <- xCellAnalysis(mat)
            saveRDS(xc, file=rds.file)
          }
          gc()
          ret <- readRDS(rds.file)
          # CD4+ -> CD4, etc. This will allow us to differentiate to RCTD
          rownames(ret) <- gsub(rownames(ret), pattern="\\+", replacement="")
          ret
        })
```
...
rctd.markers <- 
  list("B_Cells" = "MS4A1",
       ...
       "CD8..T.cells" = "CD8A",
       "Endothelial" = "PECAM1",
       "Epithelial_Basal" = "EPCAM",
       "Epithelial_Basal_Cycling" = "EPCAM",
       "Epithelial_Luminal_Mature" = "EPCAM",
       "Epithelial" = "EPCAM",
       "CAF" = "PDGFRB",
       "Stromal" = "PDGFRB",
       "Myeloid" = "CD68",
...
xcell.to.rctd <-
  list("B-cells" = "B_Cells",
       "CD4 T-cells" = "CD4..T.cells",
       "CD8 T-cells" = "CD8..T.cells",
       "Endothelial cells" = "Endothelial",
       "Epithelial cells" = "Epithelial_Basal",
       "Epithelial cells" = "Epithelial_Basal_Cycling",
       "Epithelial cells" = "Epithelial_Luminal_Mature",
...
#names(xcell.to.rctd) <- xcell.to.rctd
xcell.cells <- 1:length(xcell.to.rctd)
names(xcell.cells) <- names(xcell.to.rctd)
...
xcell.rctd.cors <-
  ldply(datasets,
        .fun = function(dataset) {
          res.xcell <-
            ldply(xcell.cells,
                  .fun = function(xcell.cell.indx) {
                    xcell.cell <- names(xcell.to.rctd)[xcell.cell.indx]
                    print(c(xcell.cell, dataset))
                    rctd.cells <- xcell.to.rctd[[xcell.cell.indx]]
                    names(rctd.cells) <- rctd.cells
                    res <- 
                      ldply(rctd.cells,
                            .fun = function(rctd.cell) {
                              # print(c(rctd.cell, xcell.cell, rctd.markers[[rctd.cell]]))
                              # print(c(xcell.cell, rctd.cell))
                              xcell.df <- data.frame(spot = colnames(xcells[[dataset]]),
                                                     xcell.val = as.numeric(xcells[[dataset]][xcell.cell,]))
                              rctd.df <- data.frame(spot = rownames(filtered.objs[[dataset]][[]]),
                                                    rctd.val = as.numeric(filtered.objs[[dataset]][[]][, rctd.cell]))
                              
                              marker.df <- data.frame(spot = colnames(l.cpms[[dataset]]),
                                                      marker.val = as.numeric(l.cpms[[dataset]][rctd.markers[[rctd.cell]],]))
                              m.df <- merge(xcell.df, rctd.df)
                              m.df <- merge(m.df, marker.df)
                              m.df <- na.omit(m.df)
                              # print(m.df)
                              data.frame(rx.cor = cor(m.df$rctd.val, m.df$xcell.val),
                                         rm.cor = cor(m.df$rctd.val, m.df$marker.val))
                            })
                    colnames(res)[1] <- "rctd.pheno"
                    res
                    
                  })
          colnames(res.xcell)[1] <- "xcell.pheno"
          res.xcell
        })
colnames(xcell.rctd.cors)[1] <- "sample"

indx <- 1
xcell.sig <- "ImmuneScore"
gene <- "COL1A1"
gene <- "PTPRC"
cor(xcells[[indx]][xcell.sig,], GetAssayData(filtered.objs[[indx]], assay="SCT", slot="data")[gene,])
```

```{r}
df <- xcell.rctd.cors
df$rctd.pheno <- gsub(df$rctd.pheno, pattern="_", replacement=" ")
g <- ggplot(data = df, aes(x = xcell.pheno, y = rx.cor))
g <- g + facet_wrap(~ rctd.pheno, scales = "free_x", labeller = label_wrap_gen(10), nrow=2)
g <- g + geom_boxplot(outlier.shape = NA)
g <- g + geom_point()
g <- g + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
g <- g + xlab("xCell Phenotype") + ylab("Pearson Correlation\n(of xCell and RCTD Phenotypes)")
text.sz <- 15
g <- g + theme(text = element_text(size = text.sz))
# To DO:
# plot good and bad spatial matches for B cells, CD4, CD8, epi, immune, and stromal.
# always include H&E.
# look at correlation plots -- are some just very low?
png(paste0(plot.dir, "/", "rctd-vs-xcell-correlation-boxplots.png"))
print(g)
d <- dev.off()
...
```{r}
df <- xcell.rctd.cors
# For simplicity, drop a few
df <- subset(df, !(rctd.pheno %in% c("Epithelial_Luminal_Mature")))
df <- subset(df, !(xcell.pheno %in% c("DC", "Monocytes")))
df$vs <- paste0(df$rctd.pheno, " vs ", df$xcell.pheno)
mean.df <- ddply(df, .variables = c("vs"), .fun = function(sub.df) data.frame(mean = mean(sub.df$rx.cor)))
mean.df <- mean.df[order(mean.df$mean),]
df$vs <- factor(df$vs, levels = mean.df$vs)
mean.sample.df <- ddply(df, .variables = c("sample"), .fun = function(sub.df) data.frame(mean = mean(sub.df$rx.cor)))
mean.sample.df <- mean.sample.df[order(mean.sample.df$mean),]
df$sample <- factor(df$sample, levels = mean.sample.df$sample)

g <- ggplot(df, aes(vs, sample, fill = rx.cor)) + 
  geom_tile() +
  scale_fill_gradient2(low="blue", mid="white", high="red", midpoint = mean(df$rx.cor)) 
g <- g + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
g <- g + xlab("") + ylab("")
g <- g + labs(fill = "RCTD / xCell\nPearson\nCorrelation")
text.sz <- 15
g <- g + theme(text = element_text(size = text.sz))
png(paste0(plot.dir, "/", "rctd-vs-xcell-correlation-heatmap.png"))
print(g)
d <- dev.off()

pts <- mean.sample.df$sample
names(pts) <- pts
hne.plts <- 
  llply(rev(pts),
        .fun = function(pt) {
          g <- plot.hne(filtered.objs[[pt]])
          title <- paste0(pt, "\n(mean cor: ", round(subset(mean.sample.df, sample == pt)$mean, 2), ")")
          g <- add.title.to.plot(g, title, rel_heights=c(0.1,1))
          g
        })

g <- plot_grid(plotlist = hne.plts)
png(paste0(plot.dir, "/", "rctd-vs-xcell-hnes.png"))
print(g)
d <- dev.off()
```
...
xcell.objs <-
  llply(datasets,
        .fun = function(sample_) {
          obj <- filtered.objs[[sample_]]
          meta.df <- t(xcells[[sample_]])
          obj <- add.metadata.to.seurat.obj(obj, meta.df)
          meta.df <- t(l.cpms[[sample_]][unique(as.character(rctd.markers)), ])
          colnames(meta.df) <- paste0(colnames(meta.df), "_")
          obj <- add.metadata.to.seurat.obj(obj, meta.df)
          obj
        })
```
...
plts <-
  llply(vses,
        ...
        .fun = function(vs_) {
          print(vs_)
          vs.df <- subset(df, vs == vs_)
          xcell.pheno <- make.names(vs.df$xcell.pheno[1])
          rctd.pheno <- make.names(vs.df$rctd.pheno[1])
          sub.plts <- 
            llply(rev(pts),
                  .parallel = FALSE,
                  .fun = function(sample_) {
                    sub.df <- subset(vs.df, sample == sample_)
                    rx.cr <- round(sub.df$rx.cor, 2)
                    rm.cr <- round(sub.df$rm.cor, 2)
                    #obj <- filtered.objs[[sample_]]
                    #meta.df <- t(xcells[[sample_]])
                    #obj <- add.metadata.to.seurat.obj(obj, meta.df)
                    obj <- xcell.objs[[sample_]]
                    # Silly underscore hack to make sure we are taking the cpm
                    ps <- plot.features_(obj, c(rctd.pheno, xcell.pheno, paste0(rctd.markers[[rctd.pheno]], "_")), include.hne=TRUE)
                    ps[[3]] <- add.title.to.plot(ps[[3]], paste0("cor w/ RCTD: ", rx.cr), rel_heights=c(0.1,1))
                    ps[[4]] <- add.title.to.plot(ps[[4]], paste0("cor w/ RCTD: ", rm.cr), rel_heights=c(0.1,1))
                    g <- plot_grid(plotlist = ps, nrow=4)
                    # g <- add.title.to.plot(g, paste0(sample_, "\n(cor: ", cr, ")"), rel_heights=c(0.1,1))
                    g <- add.title.to.plot(g, sample_, rel_heights=c(0.1,1))
                  })
          g <- plot_grid(plotlist = sub.plts, ncol = length(sub.plts))
          title <- paste0(rctd.pheno, " (RCTD) vs ", xcell.pheno, " (xCell)")
          g <- add.title.to.plot(g, title, rel_heights=c(0.1,1))
          g
        })
```
...
for(nm in names(plts)) {
  png(paste0(plot.dir, "/", "rctd-vs-xcell-", make.names(nm), ".png"), width = 3 * 480, height =
1.5 * 480)
  print(plts[[nm]])
  d <- dev.off()
}
...
```{r}
# Plot correlation of UMIs and tumor cells
plts <- 
  llply(datasets,
        .fun = function(nm) {
          g <- ggscatter(filtered.objs[[nm]][[]], x = "nCount_Spatial", 
                         y = "Epithelial_Basal_Cycling", add = "reg.line", 
                         add.params = list(color = "blue", fill = "lightgray"), 
                         conf.int = FALSE)
          # g <- g + stat_cor(method = "pearson", size=10, aes(label = ..r.label..))   
          g <- g + ylim(c(0,1))
          title <- patient.df[nm, "name"]
          g <- g + ggtitle(title) + 
            theme(plot.title = element_text(hjust = 0.5),
                  text = element_text(size = 20))
          g <- g + xlab("UMIs") + ylab("Epi Basal Cycling\nProportion")
          g <- g + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
          g
        })
g.tot <- plot_grid(plotlist = plts)
png(paste0(plots_dir, "/epi-vs-umis-scatter.png"), width = 3 * 480, height = 2 * 480)
print(g.tot)
d <- dev.off()
```
...
plts <- 
  llply(datasets,
        .fun = function(nm) {
          feat <- "CD8..T.cells"
          obj <- subset(filtered.objs[[nm]], subset = Epithelial_Basal_Cycling > 0.25)
          g <- plot.features(obj, include.hne=FALSE, include.umi.cnts=FALSE,
                          features = c(feat, "Epithelial_Basal_Cycling"),
                          feature.names = c("CD8+ T", "Epi Basal\nCycling"), nrow=1,
                          legend.limits = list(c(0,0.25),c(0,1)))
          title <- patient.df[nm, "name"]
          g <- add.title.to.plot(g, title, size = 28, rel_heights = c(0.125, 1))
          g
        })
g.tot <- plot_grid(plotlist = plts)
png(paste0(plots_dir, "/filtered-cd8-vs-epi-spatial.png"), width = 3 * 480, height = 1 * 480)
print(g.tot)
```
...
plts <- 
  llply(datasets,
        .fun = function(nm) {
          df <- filtered.objs[[nm]][[]]
          df <- subset(df, Epithelial_Basal_Cycling > 0.25)
          g <- ggscatter(df, y = "Epithelial_Basal_Cycling", 
                         x = "CD8..T.cells", add = "reg.line", 
                         add.params = list(color = "blue", fill = "lightgray"), 
                         conf.int = FALSE)
          # g <- g + stat_cor(method = "pearson", size=10, aes(label = ..r.label..))   
          g <- g + xlim(c(0,0.15)) + ylim(c(0,1))
          title <- patient.df[nm, "name"]
          g <- g + ggtitle(title) + 
            theme(plot.title = element_text(hjust = 0.5),
                  text = element_text(size = 20))
          g <- g + xlab("CD8 T Cell\nProportion") + ylab("Epi Basal Cycling\nProportion")
          g <- g + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
          g
        })
g.tot <- plot_grid(plotlist = plts)
png(paste0(plots_dir, "/cd8-vs-epi-scatter.png"), width = 3 * 480, height = 2 * 480)
print(g.tot)
d <- dev.off()
```
...
colnames(all.meta)[1] <- "sample"
all.meta <- merge(all.meta, patient.df[, c("sample", "name")])
g <- ggplot(data = subset(all.meta, Epithelial_Basal_Cycling > 0.5),
            aes(x = name, y = CD8..T.cells)) +
  geom_boxplot() +
  xlab("sample") + ylab("CD8+ T cells")
```


```{r}
# WORKING
slide3.samples <- subset(patient.df, slide == 3 & !is.na(brca1.status))
# Only keep the reseq replicate for 1863B
slide3.samples <- slide3.samples[!(slide3.samples$sample %in% c("SC2300231_JP1863B")),]
slide4.samples <- subset(patient.df, slide == 4 & !is.na(brca1.status))
slide3.samples <- slide3.samples$sample
slide4.samples <- slide4.samples$sample
names(slide3.samples) <- slide3.samples
names(slide4.samples) <- slide4.samples


plts <-
  llply(slide4.samples,
        .fun = function(nm) {
          feat <- "CD8..T.cells"
          # feat <- "Stromal"
          g <- plot.features(filtered.objs[[nm]], include.hne=TRUE, include.umi.cnts=TRUE,
                          features = c(feat, "Epithelial_Basal_Cycling"),
                          feature.names = c("CD8+ T", "Epi Basal\nCycling"), nrow=1)
          title <- patient.df[nm, "name"]
          g <- add.title.to.plot(g, title, size = 28, rel_heights = c(0.125, 1))
        })
pg <- plot_grid(plotlist=plts,nrow=length(plts))
png(paste0(plots_dir, "/slide4.png"), width = 2 * 480, height = 1.5 * 480)
print(pg)
d <- dev.off()

plts <-
  llply(slide3.samples,
        .fun = function(nm) {
          feat <- "CD8..T.cells"
          # feat <- "Stromal"
          g <- plot.features(filtered.objs[[nm]], include.hne=TRUE, include.umi.cnts=TRUE,
...
pg <- plot_grid(plotlist=plts,nrow=length(plts))
png(paste0(plots_dir, "/slide3.png"), width = 2 * 480, height = 1.5 * 480)
print(pg)
d <- dev.off()


plot.features(filtered.objs[[indx]], include.hne=TRUE, features = c("CD8..T.cells", "Epithelial_Basal_Cycling", "EPCAM"), include.umi.cnts=TRUE)
plot(filtered.objs[[indx]][[]]$CD8..T.cells, filtered.objs[[indx]][[]]$Epithelial_Basal_Cycling)

all.meta <- ldply(filtered.objs, .fun = function(obj) {
  df <- obj[[]]
  df$spot <- rownames(df)
  df
})
...
# sub <- subset(all.meta, CD8..T.cells > 0.05 | Epithelial_Basal_Cycling > 0.05)
g <- ggplot(data = all.meta, aes(x = CD8..T.cells, y = Epithelial_Basal_Cycling))
# g <- g + geom_hex() + scale_fill_continuous(type = "viridis") 
...
plts <-
  dlply(all.meta, .variables = c("name"),
        .fun = function(df) {
          g <- ggplot(data = df, aes(x = CD8..T.cells, y = Epithelial_Basal_Cycling))
          g <- g + geom_point()
          # g <- g + stat_density_2d()
          # g <- g + geom_hex()
          g <- g + xlab("CD8+ T cells") + ylab("Epithelial Basal Cycling")
          g <- g + ggtitle(df[1,"name"])
          g <- g + xlim(c(0,0.3)) + ylim(c(0,1))
          g <- ggMarginal(g, type = "density")
          g
        })

g <- plot_grid(plotlist=plts)
png(paste0(plots_dir, "/cd8-epi-densities.png"), width = 1.5 * 480, height = 1.5 * 480)
print(g)
d <- dev.off()

tmp <- human.hallmark.mcp.gsea.mats[["TNBC"]]
hallmark.cols <- grep(colnames(tmp), pattern="HALLMARK", value=TRUE)
tmp <- tmp[, c(hallmark.cols, "orig.ident", "spot")]
all.meta <- merge(all.meta, tmp)
all.meta$Epithelial_Basal_Cycling_bins <- cut(all.meta$Epithelial_Basal_Cycling, breaks =  4)
#                                                seq(0,1,by=0.1))
g <- ggplot(data = all.meta, aes(x = Epithelial_Basal_Cycling_bins, y = CD8..T.cells)) 
g <- g + geom_boxplot()
g <- g + facet_wrap(~ orig.ident)

g <- ggplot(data = all.meta, aes(x = sample, y = Myeloid)) 
g <- g + geom_boxplot()

g <- ggplot(data = all.meta, aes(x = sample, y = HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION)) 
g <- g + geom_boxplot()

g <- ggplot(data = all.meta, aes(x = sample, y = HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION)) 
g <- g + geom_boxplot()

mat <- na.omit(all.meta[,c("orig.ident", hallmark.cols,"Immune","Epithelial", "Stromal", make.names(unique(tnbc.cell.types)))])
umap.res <- umap(mat, scale=TRUE)
df <- data.frame(umap1 = umap.res[,1], umap2 = umap.res[,2], sample = mat$orig.ident)
g <- ggplot(data = df, aes(x = umap1, y = umap2, colour = sample)) + geom_point()
g <- ggplot(data = df, aes(x = umap1, y = umap2, colour = sample)) + geom_point() + facet_wrap(~ sample)

mat <- na.omit(all.meta[,c("orig.ident", hallmark.cols,"Immune","Epithelial", "Stromal", make.names(unique(tnbc.cell.types)))])
tmp <- reshape2::melt(mat, id.vars = "orig.ident")
mat <- subset(mat, !(orig.ident %in% c("SC2300233_JP6110", "SC2300232_JP6970")))
umap.res <- umap(mat, scale=TRUE)
df <- data.frame(umap1 = umap.res[,1], umap2 = umap.res[,2], sample = mat$orig.ident)
g <- ggplot(data = df, aes(x = umap1, y = umap2, colour = sample)) + geom_point()
g <- ggplot(data = df, aes(x = umap1, y = umap2, colour = sample)) + geom_point() + facet_wrap(~ sample)
