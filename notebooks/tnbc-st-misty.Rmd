...
if(!require(loomR)) {
  # This did not work.
  # Instead, I download the git repo, tgz'ed it into loomR.tgz, and then did
  # R CMD INSTALL loomR.tgz -l /home/whitebr/R/rocker-rstudio/4.0/
  # devtools::install_github(repo = "mojaveazure/loomR", ref = "develop")
}

install.scRATE <- FALSE
if(install.scRATE) {
if(!require(scRATE)) {
...
suppressPackageStartupMessages(p_load(uwot))
# for ridge in misty
dyn.load('/home/whitebr/local/gsl-2.5/lib/libgsl.so.23')

suppressPackageStartupMessages(p_load(infercnv))
...
analysis_base_dir <- orig_base_dir
...
plot.cluster.fraction.by.annotation <- function(cluster.label, annotation.label, cols = NULL) {
  ...
  g.tot <- plot_grid(
    ...
    plot_grid(NULL, NULL, get_legend(g), ncol=1, rel_heights=c(2,2,9)), 
    rel_widths=c(7,3))
...
make.cluster.plots <- function(cluster.label, sample.label, treatment.label) {
  ...
  g.treated.samples <- plot.cluster.fraction.by.annotation(sub.anno.df$cluster, sub.anno.df$sample, cols=sample_cols)

  sub.anno.df <- subset(anno.df, treatment == "Untreated")
  g.untreated.samples <- plot.cluster.fraction.by.annotation(sub.anno.df$cluster, sub.anno.df$sample, cols=sample_cols)
  ...
  return(list("treat" = g1, "samples" = g2, "treat.and.samples" = g3))
}
```
...
cluster.annos <- read.table(paste0(analysis_dir, "/cluster-anno-gsea-batch.2.tsv"), sep="\t", header=TRUE)
```

```{r}
# Split objects by cluster
clusters <- unique(cluster.annos$cluster)
names(clusters) <- clusters
clustered.objs <-
  llply(clusters,
        .fun = function(cluster.num) {
          spots.in.cluster <- subset(cluster.annos, cluster == cluster.num)
          nms <- names(filtered.objs)
          nms <- nms[unlist(lapply(nms, function(nm) nm %in% spots.in.cluster$orig.ident))]
          names(nms) <- nms
          subset.objs <-
            llply(nms,
                  .fun = function(nm) {
                    spots <- subset(spots.in.cluster, orig.ident == nm)
                    filtered.objs[[nm]][, spots$spot]
                  })
...
```{r}
run.misty.pathway.target.analysis <- function(objs, rctds, cpms, all.sample.acts, prefix = "misty-path-vs-ligand-and-cell-type-") {
  ...
  #          ~ cell type (juxta)
  #          ~ cell type (para)
  for(sample.name in names(objs)) {
    print(sample.name)
  
    results.folder <- paste0(prefix, sample.name)
    ...
    act.mat <- reshape2::dcast(sample_acts, condition ~ source)
    ...
    obj <- objs[[sample.name]]
    common.spots <- intersect(rownames(act.mat), colnames(expr))
    common.spots <- intersect(common.spots, Cells(obj))
    if(length(common.spots) < 10) {
      dir.create(results.folder)
      next
    }
    
    act.mat <- act.mat[common.spots,,drop=FALSE]
    rctd.df <- rctd.df[common.spots,,drop=FALSE]
    obj <- obj[, common.spots]
    expr <- expr[, common.spots,drop=FALSE]
...
```{r}
# Perform the misty cell type ~ cell type analysis on a per-cluster basis
run.misty.cell.type.vs.cell.type.analysis <- function(objs, rctds, cpms, all.sample.acts, prefix = "misty-cell-type-vs-cell-type-") {
  sample.name <- names(objs)[1]
  for(sample.name in names(objs)) {
    print(sample.name)
  
    # results.folder <- paste0(analysis_dir, prefix, sample.name)
    results.folder <- paste0(prefix, sample.name)
    ...
    cv.folds <- 10
    if(length(common.spots) < cv.folds) { cv.folds <- min(5, length(common.spots)-1) }
    res <- run_misty(misty.views, results.folder = results.folder, cv.folds = cv.folds)
  #%>% collect_results()
  }
}
```
...
```{r}
nms <- names(clustered.objs)
nms <- c("5", "2", "1", "3", "4", "6")
names(nms) <- nms
l_ply(nms,
      .fun = function(cluster.num) {
        print(cluster.num)
        sub.dir <- paste0(analysis_dir, "/misty-cluster-cell-type-vs-cell-type/cluster-", cluster.num, "/")
        dir.create(sub.dir, recursive=TRUE)
        run.misty.cell.type.vs.cell.type.analysis(clustered.objs[[cluster.num]], rctds, cpms, all.sample.acts, prefix = paste0(sub.dir, "misty-cluster-", cluster.num, "-cell-type-vs-cell-type-"))
      })
...
l_ply(nms,
      .fun = function(cluster.num) {
        print(cluster.num)
        sub.dir <- paste0(analysis_dir, "/misty-cluster-path-vs-ligand-and-cell-type/cluster-", cluster.num, "/")
        dir.create(sub.dir, recursive=TRUE)
        run.misty.pathway.target.analysis (clustered.objs[[cluster.num]], rctds, cpms, all.sample.acts, prefix = paste0(sub.dir, "misty-cluster-", cluster.num, "-path-vs-ligand-and-cell-type-"))
      })
```
...
#collect.all.results(prefix = paste0(analysis_dir, "cell-type-vs-cell-type"))

collect.all.results <- function(prefix, cluster.num) {
  treated.misty.dirs <- paste0(analysis_dir, "/misty-cluster-", prefix, "/cluster-", cluster.num, "/misty-cluster-", cluster.num, "-", prefix, "-", treated.samples)
  names(treated.misty.dirs) <- treated.samples
  untreated.misty.dirs <- paste0(analysis_dir, "/misty-cluster-", prefix, "/cluster-", cluster.num, "/misty-cluster-", cluster.num, "-", prefix, "-", untreated.samples)
  names(untreated.misty.dirs) <- untreated.samples

  # Make sure none of the runs crashed (and all have "importance" files, which seem to be output later in process)
  treated.flag <- unlist(lapply(treated.misty.dirs, function(dir) any(grepl(x=list.files(dir), pattern="importance"))))
  treated.misty.dirs <- treated.misty.dirs[treated.flag]
  untreated.flag <- unlist(lapply(untreated.misty.dirs, function(dir) any(grepl(x=list.files(dir), pattern="importance"))))
  untreated.misty.dirs <- untreated.misty.dirs[untreated.flag]
  
  all.samples <- c(treated.misty.dirs, untreated.misty.dirs)
  all.sample.res <- collect_results(all.samples)
  ...
  names(misty.treated.ct.individual.res) <- names(treated.misty.dirs)
  misty.untreated.ct.individual.res <- lapply(untreated.misty.dirs, function(dir) collect_results(dir))
  names(misty.untreated.ct.individual.res) <- names(untreated.misty.dirs)
  
  lst <- list("treated" = misty.treated.ct.individual.res, "untreated" = misty.untreated.ct.individual.res)
  lst.agg <- list("treated" = misty.treated.ct.res, "untreated" = misty.untreated.ct.res)
  all.ct.res <-
    ldply(lst, 
  ...
  all.ct.res <- all.ct.res[order(all.ct.res$Importance, decreasing=TRUE),]
  # for cluster.num = 2, there are plasma cell interactions
  # misty.treated.ct.res %>% plot_interaction_heatmap(view="intra")
  return(list("all.ct.res" = all.ct.res, "lst" = lst, "lst.agg" = lst.agg, "all.sample.res" = all.sample.res))
}
```
...
path.res <- 
  llply(nms, .fun = function(cluster.num) {
    collect.all.results(prefix = "path-vs-ligand-and-cell-type", cluster.num=cluster.num)
  })

ct.res <- 
  llply(nms, .fun = function(cluster.num) {
    collect.all.results(prefix = "cell-type-vs-cell-type", cluster.num=cluster.num)
  })

# path.res$all.sample.res %>% plot_interaction_heatmap(view="intra.ct")
# cluster 2 has plasma / vegf interactions
# path.res <- collect.all.results(prefix = "path-vs-ligand-and-cell-type", cluster.num="2")
```
...
```{r}
# res has entries all.ct.res, lst, lst.agg, and all.sample.res
plot.all.misty.results <- function(res, prefix, title) {
  all.res <- res$all.sample.res
  treated.res <- res$lst.agg$treated
  untreated.res <- res$lst.agg$untreated

  sample.type.res <- list("All" = all.res, "Treated" = treated.res, "Untreated" = untreated.res)
  sample.types <- names(sample.type.res)
  names(sample.types) <- sample.types
  
  views <- unique(res$all.ct.res$view)
  names(views) <- views
  view.long.names <- list("intra.lig"= "Ligand Intraview",
                          "juxta.lig_3" = "Ligand Juxtaview",
                          "para.lig_2" = "Ligand Paraview",
                          "intra.ct" = "Cell Type Intraview",
                          "juxta.ct_3" = "Cell Type Juxtaview",
                          "para.ct_2" = "Cell Type Paraview",
                          "intra" = "Intrinsic View",
                          "juxta.3" = "Juxtaview",
                          "para.2" = "Paraview")
  
  # Plot R2 and improvement in R2
  label.fun <- scales::label_wrap(15)
  plts <- llply(sample.types,
                .fun = function(sample.type) {
                  g1 <- my_plot_improvement_stats(sample.type.res[[sample.type]], measure="intra.R2") + 
                    theme(text = element_text(size = 20), 
                          axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
                          plot.title = element_text(hjust = 0.5)) +
#                    scale_x_discrete(labels = function(x) label.fun(gsub("_", " ", x, fixed=TRUE))) +
                    ylab("% Var Explained\n(Intraview)") 
                  g2 <- my_plot_improvement_stats(sample.type.res[[sample.type]], measure="gain.R2") + 
                    theme(text = element_text(size = 20), 
                          axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
                          plot.title = element_text(hjust = 0.5)) +
#                    scale_x_discrete(labels = function(x) label.fun(gsub("_", " ", x, fixed=TRUE))) +
                    ylab("Change in % Var Explained\n(Multiview vs Intraview)")
                  g <- plot_grid(NULL,g1,g2,nrow=1,rel_widths=c(0.2,1,1))
                  g <- add.title.to.plot(g, paste0(sample.type, " Samples"), rel_heights=c(0.1,1),size=20)

                })
  g.all <- plot_grid(plotlist=plts, nrow=3)
  g.all <- add.title.to.plot(g.all, title, rel_heights=c(0.1,1), size=20)

  png(paste0(prefix, "-var-explained.png"), height = 3 * 480, width = 2 * 480)
  print(g.all)
  d <- dev.off()
          ...
          .fun = function(vw) {
            sub.plts <-
              llply(sample.types,
                    .fun = function(sample.type) {
                      print(sample.type)
                      tmp <- subset(sample.type.res[[sample.type]]$importances.aggregated, view == vw)
                      trim <- 1
                      tmp <- tmp[!is.na(tmp$Importance),]
                      tmp <- subset(tmp, abs(Importance) > trim)
                      if((length(unique(tmp$Predictor)) >= 10) || (length(unique(tmp$Target)) >= 10)) { trim <- 2 }
                      g <- my_plot_interaction_heatmap(sample.type.res[[sample.type]], view = vw, trim = trim, clean = TRUE) + 
                        theme(text = element_text(size = 20), 
                              axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
                              plot.title = element_text(hjust = 0.5)) + 
                        ggtitle(paste0(view.long.names[[vw]], " (", sample.type, " samples)"))
                    })
            misty.diff.res <- merge.misty.results(sample.type.res[["Treated"]], 
                                                  sample.type.res[["Untreated"]], ".treated", ".untreated")
            tmp <- subset(misty.diff.res, view == vw)
            trim <- 1
            tmp <- tmp[!is.na(tmp$imp.diff),]
            tmp <- subset(tmp, abs(imp.diff) > trim)
            if((length(unique(tmp$Predictor)) >= 10) || (length(unique(tmp$Target)) >= 10)) { trim <- 2 }

            tmp <- subset(tmp, abs(imp.diff) > trim)
            
            g.diff <- ggplot(tmp, aes(x = Predictor, y = Target)) +
              geom_tile(aes(fill = imp.diff)) + 
              scale_fill_gradient2(low = "blue", mid = "white", 
                                   high = "red", midpoint = 0) + theme_classic() + 
              theme(text = element_text(size = 20), 
                    axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
                    plot.title = element_text(hjust = 0.5)) + 
              coord_equal() + ggtitle(paste0(view.long.names[[vw]], " (Treated - Untreated)")) +
              labs(fill = "Importance\nDifference")

            g <- plot_grid(sub.plts[["All"]], g.diff, sub.plts[["Treated"]], sub.plts[["Untreated"]],
                           labels = "AUTO", nrow=2)
            g <- add.title.to.plot(g, title, rel_heights=c(0.1,1), size=20)

            file <- paste0(prefix, "-view-", vw, "-heatmap.png")
            print(file)
            png(file, height = 2 * 480, width = 3 * 480)
            print(g)
            d <- dev.off()
          })
  ...
  plts <- llply(sample.types,
                .fun = function(sample.type) {
                  g <- my_plot_view_contributions(sample.type.res[[sample.type]]) + 
                    theme(text = element_text(size = 20), 
                          axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
                          plot.title = element_text(hjust = 0.5)) +
                    ggtitle(paste0(sample.type, " Samples"))
                })
  g.all <- plot_grid(plotlist=plts, nrow=3)
  g.all <- add.title.to.plot(g.all, title, rel_heights=c(0.1,1), size=20)

  png(paste0(prefix, "-view-contribution.png"), height = 3 * 480, width = 2 * 480)
  print(g.all)
  d <- dev.off()
...
# collect
cluster.num <- "5"
treated.misty.dirs <- paste0(analysis_dir, "misty-cluster-cell-type-vs-cell-type/cluster-", cluster.num, "/misty-cluster-", cluster.num, "-cell-type-vs-cell-type-", treated.samples)
untreated.misty.dirs <- paste0(analysis_dir, "misty-cluster-cell-type-vs-cell-type/cluster-", cluster.num, "/misty-cluster-", cluster.num, "-cell-type-vs-cell-type-", untreated.samples)

# Make sure none of the runs crashed (and all have "importance" files, which seem to be output later in process)
treated.misty.dirs <- treated.misty.dirs[unlist(lapply(treated.misty.dirs, function(dir) any(grepl(x=list.files(dir), pattern="importance"))))]
untreated.misty.dirs <- untreated.misty.dirs[unlist(lapply(untreated.misty.dirs, function(dir) any(grepl(x=list.files(dir), pattern="importance"))))]
...
for(nm in nms) {
  ...
  sub.dir <- paste0(plots_dir, "/misty-path-res-plots/")
  dir.create(sub.dir, recursive=TRUE)
  prefix <- paste0(sub.dir, "misty-path-res-cluster-", nm)
  plot.all.misty.results(path.res[[nm]], prefix = prefix, title = paste0("Cluster ", nm))
}
...
for(nm in nms) {
  ...
  sub.dir <- paste0(plots_dir, "/misty-cell-type-res-plots/")
  dir.create(sub.dir, recursive=TRUE)
  prefix <- paste0(sub.dir, "misty-cell-type-res-cluster-", nm)
  plot.all.misty.results(ct.res[[nm]], prefix = prefix, title = paste0("Cluster ", nm))
}
...
cancer.mps <- read.xlsx("41586_2023_6130_MOESM6_ESM.xlsx", sheet=1)
...
cell.type.mps <-
  llply(cell.types,
        .fun = function(ct) {
          lst <- read.xlsx("41586_2023_6130_MOESM14_ESM.xlsx", sheet=ct)
...
cross.sample.gsea.res <-
  llply(nms,
        ...
        .fun = function(nm) {
          print(nm)
          obj <- filtered.objs[[nm]]
          nm.dir <- paste0(analysis_dir, "/mp-gsea-hvg-p0/", nm)
          ...
          if(!file.exists(rds.file)) {
            ...
            all.gsea.res <-
              llply(cols,
                    .fun = function(col) {
                      ranks <- rank.mat[,col]
                      fgseaRes <- suppressWarnings(fgsea(all.mps, stats = ranks, nperm = 1000, scoreType = "pos", gseaParam = 0))
                    })
...
```{r}
lst.res <- list("batch.2" = gsea.batch.2, "batch.1" = gsea.batch.1, "repl" = gsea.repl)
lst.samples <- list("batch.2" = samples.batch.2.bias, "batch.1" = samples.batch.1.bias, "repl" = samples.replicate)
hms <- list()
column_kms <- list()
row_kms <- list()
column_kms <- list("batch.2" = 9, "batch.1" = 8, "repl" = 1)
row_kms <- list("batch.2" = 6, "batch.1" = 6, "repl" = 1)
if(TRUE) {
  nms <- names(lst.res)
#nms <- "batch.2"
for(nm in nms) {
  file <- paste0(plots_dir, "/heatmap-", nm, "-hvfs-50.tiff")
  print(file)
...
```{r}
umap.obj.repl <- prepare.seurat.objs(samples.replicate)
umap.obj.batch.1 <- prepare.seurat.objs(samples.batch.1.bias)
umap.obj.batch.2 <- prepare.seurat.objs(samples.batch.2.bias)
lst.seurat <- list("repl" = umap.obj.repl, "batch.1" = umap.obj.batch.1, "batch.2" = umap.obj.batch.2)
...
```{r}
# working
# 1. apply plot.cluster.with.hne and plot.all.clusters to sct
# 2. save these outputs
# 3. output cluster fraction plots for gsea and sct
# 4. apply rf to cluster(s) with high IGKC (cluster 1 in batch.2?)
  ...
  plts <- 
    llply(nms,
          .fun = function(sm) {
            ...
            g2 <- SpatialDimPlot(obj, cells.highlight = CellsByIdentities(object = obj, ident=cluster.num)) + scale_fill_manual(values= colors) 
