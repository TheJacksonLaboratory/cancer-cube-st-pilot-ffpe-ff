...
projects <- names(human.init.files)
...
human.mp.gsea.mats <-
  llply(projects,
        ...
        .fun = function(project) {
          ...
          suppressWarnings(process.rank.gsea.matrices(human.init.files[[project]], all.mps, output_dir))
        })
```

```{r}
combined.human.mp.gsea <- ldply(human.mp.gsea.mats)
colnames(combined.human.mp.gsea)[1] <- "project"
umap.human.mp.gsea <- umap(subset(combined.human.mp.gsea, select=-c(project, orig.ident, spot)), scale=TRUE)
df <- data.frame(umap1 = umap.human.mp.gsea[,1], umap2 = umap.human.mp.gsea[,2], project = combined.human.mp.gsea$project, sample = combined.human.mp.gsea$orig.ident)
```
...
human.hallmark.mcp.gsea.mats <-
  llply(projects,
        .parallel = FALSE,
        .fun = function(project) {
          print(project)
          output_dir <- paste0(analysis_dir, "/", project)
          suppressWarnings(process.rank.gsea.matrices(human.init.files[[project]], human.hallmark.mcp.sets, output_dir))
        })
...
file <- paste0(mouse_hallmark_plots_dir, "/", "mouse-human-breast-hallmark-mcp-gsea-heatmap", ".tiff")
...
file <- paste0(mouse_hallmark_plots_dir, "/", "mouse-human-breast-hallmark-mcp-gsea-cluster-frac.png")
w <- 2 * 2.75
png(file, width = w * 480, height = 2.75 * 480)
print(g)
d <- dev.off()
       
mouse.human.filtered.objs <- create.visium.seurat.objects(c(mouse_spaceranger_dirs, tnbc.init.info$spaceranger_dirs)[orig.idents], filter.spots = TRUE)
...
for(cluster in unique(row_clustering$cluster)) {
  print(cluster)
  g <- plot.cluster.with.hne(objs, mt.df.mod, cluster.num = cluster)
  g <- add.title.to.plot(g, paste0("Cluster ", cluster), rel_heights = c(0.1,1), size = 20)
  file <- paste0(mouse_hallmark_plots_dir, "/", "mouse-human-breast-hallmark-mcp-gsea-hne-cluster-", cluster, ".png")
  dw <- 1.5
  dh <- 2
