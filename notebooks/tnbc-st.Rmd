...
if(use.spaceranger.2.0.0.results) {
  orig_base_dir <- '/projects/compsci/jgeorge/whitebr/tnbc/orig-data-spaceranger-2.0.0/22-scbct-023/'
...
cpms <-
  llply(filtered.objs,
        ...
        .fun = function(obj) {
          id <- obj[[]]$orig.ident[1]
          rds.file <- paste0(analysis_dir, "/cpms-", id, ".rds")
          print(rds.file)
          if(!file.exists(rds.file)) {
            mat <- GetAssayData(obj, assay="Spatial", slot="counts")
            norm.mat <- edgeR::cpm(as.matrix(mat), log=FALSE)
            saveRDS(norm.mat, file = rds.file)
          }
          norm.mat <- readRDS(rds.file)
          norm.mat
        })
```
...
pdf(paste0(plots_dir, "/brca1-expr.pdf"))
for(nm in names(cpm.objs2)) {
  print(nm)
  g <- plot.features(cpm.objs2[[nm]], features=c("BRCA1", "KRT6A", "KRT18", "ACTB"), include.hne=TRUE, slot="data", include.umi.cnts=TRUE)
  g <- add.title.to.plot(g, nm, rel_heights = c(0.05, 1))
  print(g)
}
d <- dev.off()
...
for(nm in names(cpm.objs2)) {
  print(nm)
  features <- c("BRCA1", "KRT6A", "KRT18", "ACTB")
  feature.names <- paste0(features, "\n(log SCT)")
  g <- plot.features(filtered.objs[[nm]], features=features, feature.names=feature.names, include.hne=TRUE, slot="data", include.umi.cnts=TRUE)
  g <- add.title.to.plot(g, nm, rel_heights = c(0.05, 1))
  png(paste0(plots_dir, "/brca1-", nm, "-expr-cpm.png"), width = 1.4 * 480, height = 0.7 * 480)
  print(g)
  d <- dev.off()
}

for(nm in names(filtered.objs)) {
  print(nm)
  features <- c("BRCA1", "KRT6A", "KRT18", "ACTB")
  feature.names <- paste0(features, "\n(log SCT)")
  g <- plot.features(filtered.objs[[nm]], features=features, feature.names=feature.names, include.hne=TRUE, slot="data", include.umi.cnts=TRUE)
  g <- add.title.to.plot(g, nm, rel_heights = c(0.05, 1))
  png(paste0(plots_dir, "/brca1-", nm, "-expr-sct.png"), width = 1.4 * 480, height = 0.7 * 480)
  print(g)
  d <- dev.off()
}
...
for(nm in names(filtered.objs)) {
  print(nm)
  features <- c("BRCA1", "PTPRC", "IGKC")
  feature.names <- c("BRCA1", "CD45", "IGKC")
  feature.names <- paste0(feature.names, "\n(log SCT)")
  g <- plot.features(filtered.objs[[nm]], features=features, feature.names=feature.names, include.hne=TRUE, slot="data", include.umi.cnts=FALSE)
  g <- add.title.to.plot(g, nm, rel_heights = c(0.05, 1))
  png(paste0(plots_dir, "/brca1-", nm, "-expr-sct-igkc.png"), width = 1 * 480, height = 0.75 * 480)
  print(g)
  d <- dev.off()
}
...
nm <- "SC2200259_9990"
features <- c("PTPRC")
  feature.names <- c("CD45")
  feature.names <- paste0(feature.names, "\n(log SCT)")
  g <- plot.features(filtered.objs[[nm]], features=features, feature.names=feature.names, include.hne=FALSE, slot="data", include.umi.cnts=FALSE)
  png(paste0(plots_dir, "/brca1-", nm, "-expr-sct-cd45.png"))
  print(g)
  d <- dev.off()
