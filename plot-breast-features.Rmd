...
#}
# curr_datatypes=data_types[1]
curr_datatypes='pdac'
if (curr_datatypes==data_types[1]){
base_dir<-'/projects/compsci/jgeorge/USERS/somara/pdac/'
output_base_dir <- '/projects/compsci/jgeorge/whitebr/cancer-cube/pdac/'
...
base_dir <- '/projects/compsci/jgeorge/USERS/somara/breast_cancer/'
output_base_dir <- '/projects/compsci/jgeorge/whitebr/cancer-cube/breast/'
...
for(nm in names(unfiltered.objs)) {
  g <- plot.hne(unfiltered.objs[[nm]], keep.invisible.legend = TRUE)
  png(paste0(plots_dir, nm, "-hne.png"))
  print(g)
  d <- dev.off()
  
  g <- plot.spatial(unfiltered.objs[[nm]], "nFeature_Spatial", "# Genes", rescale.legend = FALSE)
  png(paste0(plots_dir, nm, "-genes.png"))
  print(g)
  d <- dev.off()
}
```
...
filtered.objs <-
  llply(filtered.objs,
        .fun = function(obj) {
          cpm.mat <- cpm(GetAssayData(obj, slot="counts", assay="Spatial"))
...
```{r}
# Get the per-spot saturation metrics
nms <- names(unfiltered.objs)
names(nms) <- nms
...
# plot(all.meta$nFeature_Spatial, all.meta$num.nonzero.genes)
```
...
```{r}
flag <- all.meta$label == "FFPE"
plot(all.meta$E[flag], all.meta$nCount_Spatial[flag])
diff <- abs(all.meta$E - all.meta$nCount_Spatial)
abline(b = 1, a = 0)
plot(all.meta$tot.reads, all.meta$total.reads)
abline(b = 1, a = 0)
```
