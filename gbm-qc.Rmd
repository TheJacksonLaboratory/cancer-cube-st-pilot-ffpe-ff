...
```{r}
# Plot H&E, UMIs, and features together
for(sample in samples) {
  g <- plot.features(unfiltered.objs[[sample]], features = NULL, include.hne = TRUE, include.umi.cnts = TRUE, include.feature.cnts = TRUE, nrow=1)
  g <- add.title.to.plot(g, sample)
  png(paste0(plots_dir, "/", analysis_file_prefix, sample, "-unfiltered-spot-spatial-qc.png"), width = 2 * 480)
  print(g)
  d <- dev.off()
}
```
...
```{r}
all.unfiltered.objs <- concatenate.seurat.objects(unfiltered.objs) 
```
...
tbl <- all.unfiltered.objs[[]]
g.cnt <- plot.distributions.vs.cell.type(tbl, response.var = "nCount_Spatial", facet.var = "orig.ident") + ylab("UMI Counts (log2)")
g.feature <- plot.distributions.vs.cell.type(tbl, response.var = "nFeature_Spatial", facet.var = "orig.ident") + ylab("Feature Counts (log2)")
