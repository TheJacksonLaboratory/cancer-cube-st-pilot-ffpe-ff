...
```{r}
lung.init.file <- '/pod/2/activities/jaxcc/CATch-project/raw-data/visium/analysis_dir/Lung_1/lungcancer.csv'
lung.dataset.info <- read.table(lung.init.file, sep=",", header=TRUE)

dataset.info <- lung.dataset.info

output_base_dir <- '/projects/compsci/jgeorge/whitebr/cancer-cube/lung-1/'
...
patient <- dataset.info$biological_replicate
short.sample <- dataset.info$names
tissue <- dataset.info$labels
...
```{r}
#nz <- rowSums(ffpe.umi)
nz <- rowSums(ffpe.umi>10)
num.z <- rowSums(ffpe.umi==0)
num.o <- rowSums(ffpe.umi==1)
df <- data.frame(nz = nz, num.z = num.z, num.o = num.o, mn = pmin(nz, num.z))
df <- subset(df, num.o < 0.05 * num.z)
df <- df[order(df$mn, decreasing=TRUE),]
```
...
gene <- "ACTB"
gene <- "EPCAM"
gene <- "Scgb1a1" # not in FFPE
gene <- "Sftpc"
gene <- "Itm2b"
gene <- "Hbb-b2"
gene <- "Hba-a2"
gene <- "Scgb3a2" # good
#genes <- c("EPCAM", "COL1A1")
gene <- "Il1b" # better
#gene <- "Pi16" # lose a low of zeros, but not normal
genes <- c(gene)
...
for(gene in genes) {
  ...
  par(mfrow=c(2,2)) 
  hist(ff.umi[gene,], main = paste("FF raw ", gene, " nzero = ", length(which(ff.umi[gene,] <= 0))), xlab = paste0(gene, " Raw Counts"))
  hist(ffpe.umi[gene,], main = paste("FFPE raw ", gene, " nzero = ", length(which(ffpe.umi[gene,] <= 0))), xlab = paste0(gene, " Raw Counts"))
  hist(ret.ff$y.res[gene,], main = paste("FF vst ", gene, " nzero = ", length(which(ret.ff$y.res[gene,] <= 0))), xlab = paste0(gene, " Raw Counts"))
  hist(ret.ffpe$y.res[gene,], main = paste("FFPE vst ", gene, " nzero = ", length(which(ret.ffpe$y.res[gene,] <= 0))), xlab = paste0(gene, " Raw Counts"))
  ...
  hist(ret.ff$y.res[gene,], main = paste("FF vst ", gene, " nzero = ", length(which(ret.ff$y.res[gene,] <= 0))), xlab = paste0(gene, " Raw Counts"))
  hist(ret.ffpe$y.res[gene,], main = paste("FFPE vst ", gene, " nzero = ", length(which(ret.ffpe$y.res[gene,] <= 0))), xlab = paste0(gene, " Raw Counts"))
  my_hist(ff.umi[gene,]/ff.umi["Actb",], xlims=c(0,5), main = paste("FF raw ", gene, " / Actb nzero = ", length(which((ff.umi[gene,]/ff.umi["Gapdh",])==0)))) 
  my_hist(ffpe.umi[gene,]/ffpe.umi["Actb",], xlims=c(0,0.5), main = paste("FFPE raw ", gene, " / Actb nzero = ", length(which((ffpe.umi[gene,]/ffpe.umi["Actb",])==0)))) 
  ...
  flag <- (ret.ff.with.ffpe.attr.umis$y[gene,]==0)
  hist(ret.ff.with.ffpe.attr.umis$y.res[gene,flag])
  plot(ret.ff$mu_0[gene,], colSums(ff.umi))
  plot(ret.ff$mu_0[gene,], ff.umi[gene,])
  ...
  par(mfrow=c(1,2))
  hist(ret.ff$pearson_residual[gene,])
  hist(ret.ff.with.ffpe.attr.umis$pearson_residual[gene,])
...
for(gene in c("EPCAM", "COL1A1")) {
  ...
  ret.ffpe <- my_correct_counts_for_gene(ffpe.vst, ffpe.umi, gene, cell_attr = ffpe.cell_attr, scale_factor = ffpe.scale_factor, verbosity = 10)
